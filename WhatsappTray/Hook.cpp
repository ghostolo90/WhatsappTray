...
static std::map<int, wchar_t*> wmTranslation = std::map<int, wchar_t*>(
{
	{ 0		, L"WM_NULL" },
	{ 1		, L"WM_CREATE" },
	{ 2		, L"WM_DESTROY" },
...
LRESULT CALLBACK CallWndRetProc(int nCode, WPARAM wParam, LPARAM lParam)
{
	if (nCode >= 0)
	{
		...
		if (msg->message == WM_SYSCOMMAND)
		{
			if (msg->wParam == 61472)
			{
				//MessageBox(NULL, L"Minimize-Command received.", L"WhatsappTray", MB_OK | MB_ICONERROR);
				...
				{
					//MessageBox(NULL, L"Minimize-Command comes from WhatsApp-Window.", L"WhatsappTray", MB_OK | MB_ICONERROR);
...
LRESULT CALLBACK CallWndRetProcLog(int nCode, WPARAM wParam, LPARAM lParam)
{
	if (nCode >= 0)
	{
		CWPRETSTRUCT *msg = (CWPRETSTRUCT*)lParam;
		...
		const wchar_t* translatedMessage = wmTranslation[msg->message];
		if (translatedMessage == NULL)
		{
			std::wostringstream translatedMessageStream;
			translatedMessageStream << L"UndefinedMessage[0x" << std::hex << msg->message << L"]";
			translatedMessage = translatedMessageStream.str().c_str();
		}
		logfile << L"\nMessage:(" << translatedMessage << L") wParam:(" << msg->wParam << L")";
...
LRESULT CALLBACK CallWndRetProcLog2(int nCode, WPARAM wParam, LPARAM lParam)
{
	if (nCode >= 0)
	{
		MessageBox(NULL, L"Message Received.", L"WhatsappTray", MB_OK | MB_ICONERROR);
	}
...
BOOL DLLIMPORT RegisterHook(HMODULE hLib, DWORD threadId, wchar_t* filepathArray)
{
	_hWndProc = SetWindowsHookEx(WH_CALLWNDPROCRET, (HOOKPROC)CallWndRetProc, hLib, threadId);
	if (_hWndProc == NULL)
	{
...
void GetPath()
{
	...
	_filepath.append(L"dll_log.txt");
}
